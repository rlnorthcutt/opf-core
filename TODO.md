...
- [x] Added `pack-doctor`: diagnoses installed packs across owned/external roots for placement-vs-trust-tier mismatches, native-tree registration problems (missing/dangling/colliding symlinks), `.opf-lock` drift, and dependency existence; reports in plain language and fixes the safe subset (move a misplaced pack, create/remove a registration symlink) after explicit approval. Never installs anything or resolves a missing dependency itself - that stays `pack-install`'s job. Backed by `scripts/pack-doctor.sh`, covered by `scripts/e2e-smoke-test.sh` including a deliberate same-run collision between two unregistered items racing for one native-tree slot.
...
- [x] Gather early-adopter feedback (two orgs already gave feedback rounds; more field data wanted) — a third round landed 2026-09-13 (a private, single-org GitLab/`pi` deployment); folded into the spec (plural entity dirs, skill-id grammar, rolling-release subsection, optional dependency `ref`, executable-type definition), schema, and tooling (curated semgrep ruleset, `ci-install-scanners.sh` apt fallback, `bump-pack-version.sh` changelog-format detection) in this pass. A follow-up post-implementation-review pass from the same deployment landed the same day: strengthened the skill name==folder rule to an ERROR, closed a real scan gap (descriptor `entrypoint`/`command` fields are executable content and are now explicitly in scan scope, with path containment enforced for `routine.json`'s `script`), enumerated which scan layers honor `scan.exclude` (content scanning only) and wired that enforcement into `install-pack.sh` for real, added skill depth-1 enforcement, and documented "CI scan templates never execute pack content" as a normative invariant. More field data still wanted.
